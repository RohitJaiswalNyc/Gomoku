...
class NeuralNetwork:
    ...
    def train_nn(self,targ):
        ...
        for state in self.state_log:
            ...
            # Compute prediction error
            out = model(x)
            targ_t = tr.from_numpy(np.full(5, targ))
            # out_flat = out.view(-1)
            loss = loss_fn(out.type(tr.FloatTensor), targ_t.type(tr.LongTensor))
